...
_REGION_ORDER = [
    config.REGION_AFRICA, config.REGION_EUROPE, config.REGION_ASIA,
    config.REGION_NORTH_AMERICA, config.REGION_SOUTH_AMERICA, config.REGION_OCEANIA,
]
...
def age_distribution_by_region(df: pd.DataFrame, path: str) -> None:
    ...
    for i, box in enumerate(bp["boxes"]):
        box.set_facecolor(_AFRICA_COLOR if regions[i] == config.REGION_AFRICA else _OTHER_COLOR)
        box.set_alpha(0.65)
    ...
    ax.set_title("Aircraft type-vintage age by destination region\n"
...
def mean_age_by_carrier_region(df: pd.DataFrame, path: str) -> None:
    ...
    tab = df.pivot_table(index="carrier_name", columns="region",
    ...
    palette = {config.REGION_AFRICA: _AFRICA_COLOR,
    ...
    for i, region in enumerate(ordered_regions):
        ...
        ax.bar(x + (i - 1.5) * width, vals, width, label=region,
               color=palette.get(region, _OTHER_COLOR), alpha=0.9)
    ax.set_xticks(x)
...
def age_vs_distance(df: pd.DataFrame, path: str) -> None:
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(comp["distance_km"], comp["type_vintage_age"], s=10, alpha=0.25,
               color=_OTHER_COLOR, label="Europe / Asia / N. America")
    ax.scatter(afr["distance_km"], afr["type_vintage_age"], s=14, alpha=0.5,
               color=_AFRICA_COLOR, label="Africa")
...
def body_type_mix_by_region(df: pd.DataFrame, path: str) -> None:
    regions = [r for r in ([config.REGION_AFRICA] + config.COMPARISON_REGIONS)
               if r in set(df["region"])]
    mix = (df[df["region"].isin(regions)]
           .groupby(["region", "body"]).size()
           .unstack(fill_value=0))
    mix = mix.reindex(index=regions)
    mix_share = mix.div(mix.sum(axis=1), axis=0)

    body_order = [c for c in ["widebody", "narrowbody", "regional_jet", "turboprop"]
                  if c in mix_share.columns]
    mix_share = mix_share[body_order]
    colors = {"widebody": "#1f618d", "narrowbody": "#5dade2",
              "regional_jet": "#f5b041", "turboprop": "#af7ac5"}

    fig, ax = plt.subplots(figsize=(10, 6))
    bottom = np.zeros(len(regions))
    for body in body_order:
        vals = mix_share[body].to_numpy()
        ax.bar(regions, vals, bottom=bottom, label=body.replace("_", " "),
               color=colors.get(body), alpha=0.9)
        bottom += vals
    ax.set_ylabel("Share of equipment assignments")
    ax.set_title("Aircraft body-type mix by destination region\n"
