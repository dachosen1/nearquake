...
def generate_earthquake_context(
    ...
    """
    Generate historical context for a significant earthquake using Gemini.
...
- Sound like an informed seismologist, not a textbook"""
    ...
    try:
        response = generate_response(prompt=prompt, role="user", model="gemini-2.0-flash")
        if response is None:
            return fallback
        # Clean up response
...
def generate_preparedness_tip() -> str:
    """
    Generate a preparedness tip using Gemini.
