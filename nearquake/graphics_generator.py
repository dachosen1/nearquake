...
COLORS = {
    ...
    'bar_colors': ['#4CAF50', '#8BC34A', '#FFC107', '#FF9800', '#F44336']  # Green to red gradient
}
...
def generate_daily_summary_graphic(
    ...
    fig.suptitle(f'Daily Earthquake Summary - {date}',
    ...
    bars = ax1.bar(mag_labels, mag_counts, color=COLORS['bar_colors'],
    ...
    ax1.set_xlabel('Magnitude Range', fontsize=16, color=COLORS['text'],
                   fontweight='500', labelpad=10)
    ax1.set_ylabel('Earthquakes', fontsize=16, color=COLORS['text'],
                   fontweight='500', labelpad=10)
    ...
    # Total events
    ax2.text(0.1, stats_y, 'Total Earthquakes',
             fontsize=15, color=COLORS['text_light'], fontweight='500')
    ax2.text(0.1, stats_y - 0.13, f'{total_count:,}',
             fontsize=43, fontweight='700', color=COLORS['primary'])
    stats_y -= line_height * 1.6

    # Largest quake
    ax2.text(0.1, stats_y, 'Largest Earthquake',
             fontsize=15, color=COLORS['text_light'], fontweight='500')
    ax2.text(0.1, stats_y - 0.13, f'M{max_mag:.1f}',
             fontsize=38, fontweight='700', color=COLORS['accent'])
    ax2.text(0.1, stats_y - 0.24, location_short,
             fontsize=14, color=COLORS['text_light'], style='italic',
             wrap=True)

    # Footer
    fig.text(0.98, 0.02, 'Data: USGS | @quakebot_',
             ha='right', fontsize=8, color=COLORS['text_light'])

    plt.tight_layout(rect=[0, 0.03, 1, 0.93])

    # Save to bytes
    buf = io.BytesIO()
    plt.savefig(buf, format='png', facecolor=COLORS['bg'], dpi=300,
...
def generate_weekly_summary_graphic(
    ...
    log_info(_logger, f"Generating weekly summary graphic from {start_date} to {end_date}")
    ...
    fig.suptitle(f'Weekly Earthquake Summary - {start_fmt} to {end_fmt}',
                 fontsize=24, fontweight='700', color=COLORS['text'],
                 x=0.5, y=0.96)

    gs = fig.add_gridspec(2, 2, height_ratios=[1, 1], width_ratios=[4.5, 1],
    ...
    ax1.plot(day_labels, daily_counts,
             marker='o', linewidth=2.5, markersize=8,
             color=COLORS['primary'], markerfacecolor=COLORS['accent'],
             markeredgewidth=0)
    ax1.fill_between(range(len(day_labels)), daily_counts,
                     alpha=0.15, color=COLORS['primary'])

    ax1.set_ylabel('Daily Earthquakes', fontsize=16, color=COLORS['text'],
                   fontweight='500', labelpad=10)
    ax1.tick_params(colors=COLORS['text_light'], labelsize=14)
    ax1.spines['top'].set_visible(False)
    ax1.spines['right'].set_visible(False)
    ...
    bars = ax2.bar(mag_labels, mag_counts, color=COLORS['bar_colors'],
                   edgecolor='none', alpha=0.9, width=0.6)
    ...
    ax2.set_xlabel('Magnitude Range', fontsize=14, color=COLORS['text'],
                   fontweight='500', labelpad=8)
    ax2.set_ylabel('Earthquakes', fontsize=14, color=COLORS['text'],
                   fontweight='500', labelpad=8)
    ...
    # Total events
    ax3.text(0.1, stats_y, 'Total Earthquakes',
             fontsize=15, color=COLORS['text_light'], fontweight='500')
    ax3.text(0.1, stats_y - 0.10, f'{total_count:,}',
             fontsize=43, fontweight='700', color=COLORS['primary'])
    stats_y -= line_height * 1.6

    # Largest quake
    ax3.text(0.1, stats_y, 'Largest Earthquake',
             fontsize=15, color=COLORS['text_light'], fontweight='500')
    ax3.text(0.1, stats_y - 0.10, f'M{max_mag:.1f}',
             fontsize=38, fontweight='700', color=COLORS['accent'])
    ax3.text(0.1, stats_y - 0.18, location_short,
             fontsize=14, color=COLORS['text_light'], style='italic',
             wrap=True)
