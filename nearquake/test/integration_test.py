...
class TestDataProcessingIntegration:
    ...
    def test_location_data_pipeline(self, test_db):
        ...
        with patch("nearquake.data_processor.fetch_json_data_from_url", return_value=sample_location_response):
            # Create and run the uploader
            ...
            with patch("nearquake.data_processor.backfill_valid_date_range") as mock_backfill:
    ...
    def test_tweet_pipeline(self, test_db):
        ...
        # Configure mock query to return our test event
        mock_event = MockEventDetails(**test_event_data)
        test_db.session.query.return_value.join.return_value.filter.return_value.filter.return_value.all.return_value = [mock_event]
...
def test_end_to_end_pipeline(test_db):
    ...
    with patch.object(UploadEarthQuakeEvents, "upload") as mock_earthquake_upload, \
