...
def test_generate_response_with_system_role(mock_create):
    ...
    # Call the function with role="role" (which should be treated as system)
    result = generate_response("System prompt", "role", "gpt-4o-mini")
    ...
    mock_create.assert_called_once()
    call_args = mock_create.call_args[1]
    assert call_args["messages"][0]["role"] == "role"
...
def test_generate_response_invalid_role():
    ...
    with pytest.raises(
        ValueError, match="Error: Invalid role. Please choose 'role' or 'user'."
    ):
